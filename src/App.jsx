...
// ─── Pixel-art tamagotchi sheep — 7 expressions, pure SVG ────────────────────
function Mascot({expr="idle", size=200}){
  ...
  // ── palette ──────────────────────────────────────────────────────────────
  const w1="#4CAF50", w2="#66BB6A", w3="#2E7D32";   // wool
  const h1="#8D6E63", h2="#5D4037";                  // horn
  const f1="#76C47E";                                // face
  const ey="#1A1A2E", sh="#FFFFFF";                  // eye / shine
  const no="#5D4037";                                // nose
  const mo="#3E2723";                                // mouth
  const bl="#FF8A80";                                // blush
  const te="#64B5F6";                                // tear
  const gd="#FFD700";                                // gold
  const ht="#78909C", h2t="#546E7A";                 // hat
  ...
  const Body=()=>(
    <g>
      ...
      <rect x="11" y="2"  width="2" height="7"  rx="1"   fill={h2} opacity="0.45"/>
      <rect x="26" y="2"  width="2" height="7"  rx="1"   fill={h2} opacity="0.45"/>
      {/* Ears */}
      <rect x="1"  y="14" width="7" height="11" rx="3.5" fill={w1}/>
      <rect x="32" y="14" width="7" height="11" rx="3.5" fill={w1}/>
      <rect x="2"  y="15" width="5" height="9"  rx="2.5" fill={bl} opacity="0.55"/>
      <rect x="33" y="15" width="5" height="9"  rx="2.5" fill={bl} opacity="0.55"/>
      {/* Wool body */}
      ...
      {/* Dark outline shadow */}
      <rect x="5"  y="8"  width="30" height="26" rx="13" fill={w3} opacity="0.18"/>
      {/* Face oval */}
      <rect x="10" y="16" width="20" height="20" rx="9"  fill={f1}/>
      {/* Nose */}
      <rect x="17" y="29" width="6" height="3" rx="1.5" fill={no}/>
      <circle cx="19" cy="30" r="1" fill={h2} opacity="0.55"/>
      <circle cx="21" cy="30" r="1" fill={h2} opacity="0.55"/>
    </g>
  );
  ...
  const Expr={
    idle:(
      <g>
        ...
        <path d="M15 33 Q20 36.5 25 33" stroke={mo} strokeWidth="1.8" fill="none" strokeLinecap="round"/>
      </g>
    ),
    happy:(
      <g>
    ...
    celebrating:(
      <g>
        ...
        {/* Gold stars */}
        <text x="4"  y="13" fontSize="7" fill={gd} style={{fontFamily:"monospace"}}>★</text>
        <text x="30" y="13" fontSize="7" fill={gd} style={{fontFamily:"monospace"}}>★</text>
        <text x="2"  y="26" fontSize="5" fill={gd} style={{fontFamily:"monospace"}}>✦</text>
        <text x="35" y="26" fontSize="5" fill={gd} style={{fontFamily:"monospace"}}>✦</text>
        {/* Blush */}
    ...
    legend:(
      <g>
        ...
        {/* Sparkles */}
        <text x="2"  y="21" fontSize="7" fill={gd} style={{fontFamily:"monospace"}}>✦</text>
        <text x="32" y="18" fontSize="6" fill={gd} style={{fontFamily:"monospace"}}>★</text>
      </g>
    ),
  };
    ...
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      {glow&&(
        <div style={{
          ...
          borderRadius:"50%",
          background:`radial-gradient(circle,${expr==="legend"?"rgba(255,200,0,0.45)":"rgba(88,204,2,0.35)"} 0%,transparent 65%)`,
